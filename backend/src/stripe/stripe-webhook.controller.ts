...
export class StripeWebhookController {
  ...
  async handleBillingWebhook(
    ...
    if (!webhookSecret) throw new BadRequestException('Billing webhook not configured');

    let event: StripeEvent;
    try {
      const stripe = new Stripe(process.env.STRIPE_BILLING_SECRET_KEY || '', { apiVersion: '2026-04-22.dahlia' });
      event = stripe.webhooks.constructEvent(req.rawBody!, sig, webhookSecret);
    } catch {
      throw new BadRequestException('Billing webhook signature verification failed');
    }

    if (event.type === 'checkout.session.completed') {
      const session = event.data.object as { mode?: string | null; metadata?: Record<string, string> | null; customer?: string | null; subscription?: string | null };
