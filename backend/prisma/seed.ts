...
async function main() {
  ...
  const events = await Promise.all([
    prisma.event.create({
      data: {
        ...
        description:
          '初心者歓迎のバドミントン交流会です。ラリー中心でゆるく体を動かしながら、20代同士で自然に話せる雰囲気を大事にしています。',
    ...
    prisma.event.create({
      data: {
        ...
        title: '初心者歓迎フットサル会',
        description:
          '経験者だけで固まらない、20代中心のフットサル会です。運動不足解消や友達作りでの参加も歓迎です。',
    ...
    prisma.event.create({
      data: {
        ...
        title: '20代ミックスバスケ',
        description:
          'ブランクありでも参加しやすいバスケ交流会です。軽いアップ、チーム分け、ゲームの流れで進めます。',
    ...
    prisma.event.create({
      data: {
        ...
        title: '初参加向けバドミントン会',
        description:
          '初参加の人が多かった過去開催イベントです。参加者の声をSEOページやイベント詳細で使う想定のダミーデータです。',
  ...
  const reservations: { id: string; eventId: string; memberId: string; status: ReservationStatus; waitlistOrder?: number }[] = [
