...
function SeoTextSection({
  ...
  async function handleSave() {
    ...
    setError('');
    try {
      await api.tenant.update({ reviewsSeoDescription: value.trim() });
      onSaved(value.trim());
      setSaved(true);
  ...
  return (
    <div className="mb-6 max-w-3xl rounded-xl border border-gray-200 bg-gray-50 p-4 md:p-5">
      <p className="mb-1 text-xs font-medium text-gray-700">口コミページのSEO文言</p>
      <p className="mb-3 text-xs leading-relaxed text-gray-500">
        公開用の口コミページの見出し下とmeta descriptionに使われます。空欄の場合は団体の紹介文が使われます。「評判」「口コミ」で検索して不安を持っている人に向けて、団体の雰囲気や安心して参加できる理由を書くのがおすすめです。
      </p>
      <textarea
        value={value}
...
export default function AdminReviewsPage() {
  ...
  return (
    <div className="px-4 py-4 md:px-6 md:py-6">
      ...
      {seoDescription !== null && (
        <SeoTextSection initialValue={seoDescription} onSaved={setSeoDescription} />
      )}
