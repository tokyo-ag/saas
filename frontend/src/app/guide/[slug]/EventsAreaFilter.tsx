...
function computeEventAreas(events: PublicArticleEvent[]): string[] {
  ...
  for (const event of events) {
    for (const tag of event.tags ?? []) {
      if (LOCATION_TAG_SET.has(tag)) countByArea.set(tag, (countByArea.get(tag) ?? 0) + 1);
    }
  }
...
export default function EventsAreaFilter({ events }: { events: PublicArticleEvent[] }) {
  const [selectedArea, setSelectedArea] = useState<string | null>(null);
  const areas = computeEventAreas(events);
